...
def update_treeview():
    for row in tree.get_children():
        tree.delete(row)
    conn.execute("SELECT TID, NAME FROM TEACHER")
...
def parse_data():
    ...
    conn.execute(f"REPLACE INTO TEACHER (TID ,NAME)\
        VALUES ('{fid}','{name}')")
    cnx.commit()
    update_treeview()
    ...
    try:
        if len(tree.selection()) > 1:
            messagebox.showerror(
                "Bad Select", "Select one teacher at a time to update!")
            return

        q_fid = tree.item(tree.selection()[0])['values'][0]
        conn.execute(f"SELECT * FROM TEACHER WHERE TID = '{q_fid}'")
        cursor = conn.fetchall()
        cursor = list(cursor)
        fid_entry.insert(0, cursor[0][0])
        name_entry.insert(0, cursor[0][1])

        conn.execute(f"DELETE FROM TEACHER WHERE TID = '{cursor[0][0]}'")
        cnx.commit()
        update_treeview()
...
def remove_data():
    ...
    for i in tree.selection():
        ...
        conn.execute(
            f"DELETE FROM TEACHER WHERE TID = '{tree.item(i)['values'][0]}'")
        cnx.commit()
        tree.delete(i)
        update_treeview()
